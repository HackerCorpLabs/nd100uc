...
exports.registerBitView = registerBitView;
  ...
  <div>Place cursor in a .uc microcode file<br>to see the bit breakdown.</div>
    ...
    for (let row = 0; row < 8; row++) {
        ...
        for (let i = 0; i < 8; i++) {
            ...
            rowBits.push(`<div class="bit ${bit === "1" ? "set" : "clear"}" title="bit ${bitIdx}">${bitIdx}</div>`);
        }
        bitGrid.push(`<div class="bit-row"><span class="bit-range">${hi}..${lo}</span>${rowBits.join("")}</div>`);
    }
    // Field table.
    const rows = [];
    for (const f of FIELDS) {
        ...
        rows.push(`<tr class="color-${f.color}">` +
...
class BitViewController {
    ...
    open(context) {
        ...
        this.panel = vscode.window.createWebviewPanel(BIT_VIEW_VIEW_TYPE, "ND MicroCode Bit View", { viewColumn: vscode.ViewColumn.Beside, preserveFocus: true }, { enableScripts: false, retainContextWhenHidden: true });
        ...
        this.disposables.push(vscode.window.onDidChangeTextEditorSelection((e) => this.refresh(e.textEditor)), vscode.window.onDidChangeActiveTextEditor((e) => this.refresh(e)), vscode.workspace.onDidChangeTextDocument((e) => {
    ...
    refresh(editor, force = false) {
        ...
        if (key === this.lastInstructionKey && !force)
            return;
    ...
    computeState(editor) {
        ...
        const diagnostics = [];
        const canonical = (0, microcode_1.translateInstruction)(target, pack, sm.labels, diagnostics);
        const storage = model === "nd110" ? canonical ^ 0x0fc00000n : canonical;
        const fields = (0, microcode_1.decodeFields)(canonical, target.address);
        return {
    ...
    loadPack(model) {
        ...
        if (cached)
            return cached;
        try {
            const pack = (0, loadDefsEmbedded_1.loadEmbeddedDefinitions)(model);
...
function registerBitView(context) {
    return vscode.commands.registerCommand("nd-microcode.openBitView", () => controller.open(context));
}
