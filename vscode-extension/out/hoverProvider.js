...
const token_hover_json_1 = __importDefault(require("./generated/token-hover.json"));
const data = token_hover_json_1.default;
...
function wordRangeAtPosition(document, position) {
    ...
    while (start > 0 && TOKEN_CHAR_RE.test(line[start - 1]))
        start--;
    while (end < line.length && TOKEN_CHAR_RE.test(line[end]))
        end++;
    if (start === end)
        return undefined;
...
function parseNumeric(s) {
    ...
    if (s[0] === "o" || s[0] === "O")
        return Number.parseInt(s.slice(1), 8);
    if (/^[0-7]+$/.test(s))
...
const COMM_CONSUMES_IDB_FALLBACK = {
...
function aludDescText(name) {
    ...
    if (entry) {
        const parts = [];
        if (entry.b_register_write)
            parts.push(entry.b_register_write);
        else if (entry.q_update)
            parts.push(entry.q_update);
        else if (entry.y_bus)
            parts.push(`Y = ${entry.y_bus}`);
        if (entry.note)
...
function interpretNumeric(value, siblingTokens) {
    ...
    const aRegToken = siblingTokens.find((t) => /^A,/.test(t) && !t.startsWith("ALU"));
    const bRegToken = siblingTokens.find((t) => /^B,/.test(t) && !t.startsWith("ALU"));
...
function formatTokenHover(entry) {
    ...
    const showModelWarnings = config.get("hover.showModelWarnings", true);
    ...
    md.appendMarkdown(`*Category: \`${entry.category}\`* &nbsp;&middot;&nbsp; ` +
        `*${lineNames[entry.displayLine] ?? "Line ?"}*\n\n`);
    if (entry.provides || entry.requires) {
        const parts = [];
        if (entry.provides)
            parts.push(`provides ${entry.provides}`);
        if (entry.requires)
            parts.push(`requires ${entry.requires}`);
        md.appendMarkdown(`*${parts.join(", ")}*\n\n`);
    }
    if (showEncoding) {
        const { w4, w3, w2, w1 } = entry.encoding;
        md.appendMarkdown("| w4 (63:48) | w3 (47:32) | w2 (31:16) | w1 (15:0) |\n" +
            "|:--:|:--:|:--:|:--:|\n" +
            `| \`${w4}\` | \`${w3}\` | \`${w2}\` | \`${w1}\` |\n`);
    }
    return md;
...
function registerHoverProvider() {
    return vscode.languages.registerHoverProvider({ language: "uc" }, {
        provideHover(document, position) {
            ...
            if (NUMERIC_RE.test(word)) {
                ...
                return new vscode.Hover(formatNumericHover(word, value, siblings), range);
            }
            return undefined;
        },
    });
